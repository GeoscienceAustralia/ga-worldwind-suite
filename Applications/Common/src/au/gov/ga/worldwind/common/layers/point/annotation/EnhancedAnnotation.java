...
public class EnhancedAnnotation extends GlobeAnnotation
{
	...
	protected void doRenderNow(DrawContext dc)
	{
		...
		double scale = WWMath.clamp(distanceFactor, this.attributes.getDistanceMinScale(), this.attributes.getDistanceMaxScale());
		...
		this.drawTopLevelAnnotation(dc, (int) screenPoint.x, (int) screenPoint.y, size.width, size.height, scale, opacity, pos);
	}

	/**
	 * Compute the opacity of the annotation based on the distance between the eye and the marker point
	 * <p/>
	...
	{
		return getEnhancedAttributes().getMinEyeDistance() != null && eyeDistance < getEnhancedAttributes().getMinEyeDistance();
	}
	
	private boolean isGreaterThanMaxDistance(double eyeDistance)
	{
		return getEnhancedAttributes().getMaxEyeDistance() != null && eyeDistance > getEnhancedAttributes().getMaxEyeDistance();
	}
