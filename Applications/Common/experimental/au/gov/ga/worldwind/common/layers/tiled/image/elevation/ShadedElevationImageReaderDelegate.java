...
public class ShadedElevationImageReaderDelegate implements ImageReaderDelegate
{
	...
	protected final double exaggeration = 0.0001;
	protected final Vec4 sunPosition = new Vec4(-1, -1, -1).normalize3();
	...
	public BufferedImage readImage(TextureTile tile, URL url, Globe globe) throws IOException
	{
		...
		/*double[] minmax = getMinMax(elevations, missingDataSignal);
		...
		for (int y = 0, i = 0; y < height; y++)
		{
			...
			{
				int value =
						(int) (255d * (elevations.getDouble(i) - minmax[0])
								/ (minmax[1] - minmax[0]) * 1);
				value %= 256;
				int rgb = 0xff << 24 | value << 16 | value << 8 | value;
