...
import nasa.worldwind.util.AtmosphericScatteringComputer;
...
public class AtmosphereLayer extends AbstractLayer
{
                                ...
                                int slices, int stacks)
    {
        ...
        for (int slice = 0; slice <= slices; slice++)
        {
            ...
            color = this.asc.getAtmosphereColor(v2.transformBy4(skyTransform), eyePoint, this.lightDirection);
        ...
        for (int stack = 1; stack < stacks - 1; stack++)
        {
            ...
            for (int slice = 0; slice <= slices; slice++)
            {
                ...
                color = this.asc.getAtmosphereColor(v.transformBy4(skyTransform), eyePoint, this.lightDirection);
                gl.glColor4f(color.getRed() / 255f, color.getGreen() / 255f, color.getBlue() / 255f, color.getAlpha() / 255f);
                gl.glVertex3d(v.getX(), v.getY(), v.getZ());
                stackColors[slice] = color;
            }
