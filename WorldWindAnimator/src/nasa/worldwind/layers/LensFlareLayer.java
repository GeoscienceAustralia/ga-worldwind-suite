...
public class LensFlareLayer extends RenderableLayer
{
	public static class FlareImage extends ScreenAnnotation
	{
		...
		public FlareImage(BufferedImage image, double scale, double position,
				double opacity)
		{
			super("", new Point(0, 0));
			this.image = image;
		...
		private void initialize()
		{
			...
			aa.setSize(new Dimension(this.image.getWidth(), this.image
					.getHeight()));
	...
	private static double SUN_DISTANCE = 149597892e3;// * 1/100000d;

	private Vec4 sunDirection;
	private Vec4 sunPoint;
	private double sunDistance = SUN_DISTANCE;
	...
	public void setSunDirection(Vec4 direction)
	{
		if (direction != null)
		{
			...
			this.sunPoint = this.sunDirection.multiply3(sunDistance);
		}
		else
		{
	...
	public static final String PRESET_NOSUN = "LensFlare.PresetNoSun";
	...
	public static LensFlareLayer getPresetInstance(String preset)
	{
		...
		if (PRESET_BOLD.equals(preset))
		{
			...
			lensFlareLayer.addRenderable(new FlareImage(sun, .6, 0, .5));
			lensFlareLayer.addRenderable(new FlareImage(halo, 1.0, 0, .4));
			lensFlareLayer.addRenderable(new FlareImage(halo, 1.8, 0, .2));
			lensFlareLayer.addRenderable(new FlareImage(halo, 3, 0, .1));
		...
		else if (PRESET_NOSUN.equals(preset))
		{
			lensFlareLayer.addRenderable(new FlareImage(disk, .1, .4, .1));
			lensFlareLayer.addRenderable(new FlareImage(disk, .15, .6, .1));
			lensFlareLayer.addRenderable(new FlareImage(disk, .2, .7, .1));
	...
	public static BufferedImage createHaloImage(int size, Color color)
	{
		BufferedImage image = PatternFactory.createPattern(
				PatternFactory.PATTERN_CIRCLE, new Dimension(size, size), .6f,
				color);
	...
	public static BufferedImage createRaysImage(int size, int rays, Color color)
	{
		...
		Color c2 = new Color(color.getRed(), color.getGreen(), color.getBlue(),
				0);
		GradientPaint gradient = new GradientPaint(half, half, color, half,
				half - r1, c2);
	...
	public static BufferedImage createRainbowImage(int size)
	{
		...
		for (float r = r1; r <= r2; r++)
		{
			...
			g2.drawOval((int) (half - r), (int) (half - r), (int) (r * 2),
					(int) (r * 2));
		}
		return image;
	}
