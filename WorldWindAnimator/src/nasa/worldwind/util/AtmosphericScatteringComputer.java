...
public class AtmosphericScatteringComputer
{
    ...
    private float Kr = 0.0025f;		// Rayleigh scattering constant
    ...
    public Color getAtmosphereColor(Vec4 vPos, Vec4 vCamera, Vec4 vLightDirection)
    {
        ...
        if (fDet >= 0)
        {
            ...
            if (fNear1 >= 0 || fNear2 >= 0)
            {
                ...
                vPos = vCamera.add3(vRay.multiply3(fNear));
                color = getColorForVertex(vPos, vCamera, vLightDirection);
            }
        }
        return color;
    }
