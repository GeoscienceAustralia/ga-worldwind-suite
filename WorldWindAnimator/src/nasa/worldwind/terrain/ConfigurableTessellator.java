...
/**
 * @author tag
 * @version $Id: RectangularTessellator.java 8705 2009-02-02 00:33:12Z dcollins
 *          $
 */
public class ConfigurableTessellator extends WWObjectImpl implements
		Tessellator
{
	...
	{
		private final ConfigurableTessellator tessellator;
		...
		public RectTile(ConfigurableTessellator tessellator, Extent extent,
		...
		public ExtractedShapeDescription getIntersectingTessellationPieces(
				Plane[] p)
		{
		...
		public ExtractedShapeDescription getIntersectingTessellationPieces(
				Vec4 Cxyz, Vec4 uHat, Vec4 vHat, double uRadius, double vRadius)
		{
			return this.tessellator.getIntersectingTessellationPieces(this,
					Cxyz, uHat, vHat, uRadius, vRadius);
		}
	}
	...
	private static final String CACHE_ID = RectangularTessellator.class
			.getName();
	...
	private double elevationOffset = 0;
	...
	public SectorGeometryList tessellate(DrawContext dc)
	{
		...
		this.maxLevel = Configuration.getIntegerValue(
				AVKey.RECTANGULAR_TESSELLATOR_MAX_LEVEL, DEFAULT_MAX_LEVEL);
	...
	{
		Vec4[] corners = tile.sector.computeCornerPoints(dc.getGlobe(), dc
				.getVerticalExaggeration());
		Vec4 centerPoint = tile.sector.computeCenterPoint(dc.getGlobe(), dc
				.getVerticalExaggeration());
	...
	private double computeTileResolutionTarget(DrawContext dc, RectTile tile)
	{
		...
		double hint = dc.getGlobe().getElevationModel().getDetailHint(
				tile.sector);
		return DEFAULT_LOG10_RESOLUTION_TARGET + hint;
	}
	...
	private ConfigurableTessellator.CacheKey createCacheKey(DrawContext dc,
			RectTile tile)
	{
	...
	private void makeVerts(DrawContext dc, RectTile tile)
	{
		...
		if (tile.ri != null
				&& tile.ri.time >= System.currentTimeMillis() - 1000) // Regenerate cache after one second
			return;
	...
	private long render(DrawContext dc, RectTile tile, int numTextureUnits)
	{
		...
		for (int i = 0; i < numTextureUnits; i++)
		{
			...
			if (texCoords != null && texCoords instanceof DoubleBuffer)
				...
				gl.glTexCoordPointer(2, GL.GL_DOUBLE, 0, tile.ri.texCoords
						.rewind());
		}
			...
			boolean showTriangles, boolean showTileBoundary)
	{
		...
		if (showTriangles)
		{
			...
			gl.glVertexPointer(3, GL.GL_DOUBLE, 0, tile.ri.vertices.rewind());
			gl.glDrawElements(javax.media.opengl.GL.GL_TRIANGLE_STRIP,
					tile.ri.indices.limit(),
					javax.media.opengl.GL.GL_UNSIGNED_INT, tile.ri.indices
							.rewind());
			...
			Point pickPoint)
	{
		...
		tile.ri.vertices.position(3 * indices[0]);
		tile.ri.vertices.get(coords);
		Vec4 v0 = new Vec4(coords[0] + centerX, coords[1] + centerY, coords[2]
				+ centerZ);
		...
		tile.ri.vertices.position(3 * indices[1]);
		tile.ri.vertices.get(coords);
		Vec4 v1 = new Vec4(coords[0] + centerX, coords[1] + centerY, coords[2]
				+ centerZ);
		...
		tile.ri.vertices.position(3 * indices[2]);
		tile.ri.vertices.get(coords);
		Vec4 v2 = new Vec4(coords[0] + centerX, coords[1] + centerY, coords[2]
				+ centerZ);
	 ...
	 *         distance from the line origin, or null if no intersection was
	 *         found.
	 */
	private Intersection[] intersect(RectTile tile, Line line)
	{
		...
		Plane horizontalPlane = new Plane(normalH.x(), normalH.y(),
				normalH.z(), -line.getOrigin().dot3(normalH));
		...
		double cellSide = tile.getSector().getDeltaLatRadians()
		...
		double elevationSpan = ((Cylinder) tile.getExtent())
				.getCylinderHeight();
		...
		for (int i = startIndice; i < endIndice; i += 2)
		{
			...
			Vec4 v1 = new Vec4(coords[vIndex++] + centerX, coords[vIndex++]
					+ centerY, coords[vIndex] + centerZ);

			vIndex = 3 * indices[i + 2];
			Vec4 v2 = new Vec4(coords[vIndex++] + centerX, coords[vIndex++]
					+ centerY, coords[vIndex] + centerZ);
			...
			Vec4 p;
			vIndex = 3 * indices[i];
			Vec4 v0 = new Vec4(coords[vIndex++] + centerX, coords[vIndex++]
					+ centerY, coords[vIndex] + centerZ);

			vIndex = 3 * indices[i + 3];
			Vec4 v3 = new Vec4(coords[vIndex++] + centerX, coords[vIndex++]
					+ centerY, coords[vIndex] + centerZ);
	 ...
	 * <code>Intersection</code> for each, corresponding to each geometry
	 * triangle that intersects the given elevation.
	 * 
	 * @param tile
	 *            the <Code>RectTile</code> which geometry is to be tested for
	 *            intersection.
	...
	private Intersection[] intersect(RectTile tile, double elevation)
	{
		...
		if (!(globe
				.isPointAboveElevation(cylinder.getBottomCenter(), elevation) ^ globe
	...
	private static double[] baryCentricCoordsRequireInside(Vec4 pnt, Vec4[] V)
	{
		...
		double triangleHeight = distanceFromLine(V[0], V[1], V[2]
				.subtract3(V[1]));
		double heightFromPoint = distanceFromLine(pnt, V[1], V[2]
				.subtract3(V[1]));
	...
	private SectorGeometry.ExtractedShapeDescription getIntersectingTessellationPieces(
			RectTile tile, Plane[] planes)
	{
		tile.ri.vertices.rewind();
		tile.ri.indices.rewind();
		...
		for (int i = 0; i < trianglesNum; i++)
		{
			...
			for (int j = 0; j < 3; j++)
			{
				...
				tile.ri.vertices.get(coords);
				triVerts[j] = new Vec4(coords[0] + offset.getX(), coords[1]
						+ offset.getY(), coords[2] + offset.getZ(), 1.0);
			}
			clippedTriangleList = addClippedPolygon(triVerts, planes,
					clippedTriangleList);
		}
		return clippedTriangleList;
	}

	private SectorGeometry.ExtractedShapeDescription addClippedPolygon(
			Vec4[] triVerts, Plane[] planes,
			SectorGeometry.ExtractedShapeDescription l)
	{
		...
		if (l == null)
			l = new SectorGeometry.ExtractedShapeDescription(
					new ArrayList<Vec4[]>(),
					new ArrayList<SectorGeometry.BoundaryEdge>());
	...
	private boolean isSkirt(Vec4[] triVerts)
	{
		...
		double maxC = Math
				.max(Math.abs(triVerts[0].x), Math.abs(triVerts[0].y));
	...
	private SectorGeometry.ExtractedShapeDescription getIntersectingTessellationPieces(
			RectTile tile, Vec4 Cxyz, Vec4 uHat, Vec4 vHat, double uRadius,
			double vRadius)
	{
		tile.ri.vertices.rewind();
		tile.ri.indices.rewind();
		...
		for (int i = 0; i < trianglesNum; i++)
		{
			...
			clippedTriangleList = addClippedPolygon(triVerts, Cxyz, uHat, vHat,
					uRadius, vRadius, clippedTriangleList);
		}
		return clippedTriangleList;
	}

	private SectorGeometry.ExtractedShapeDescription addClippedPolygon(
			Vec4[] polyVerts, Vec4 Cxyz, Vec4 uHat, Vec4 vHat, double uRadius,
			double vRadius, SectorGeometry.ExtractedShapeDescription l)
	{
		// ignore triangles on skirts
		...
		for (Vec4 vtx : polyVerts)
		{
			...
			double halfspaceEqn = (xd * xd) / (uRadius * uRadius) + (yd * yd)
					/ (vRadius * vRadius) - 1.0;
			if (halfspaceEqn <= 0.0)
			{
				locIn = i++;
		...
		SectorGeometry.BoundaryEdge be = new SectorGeometry.BoundaryEdge(null,
				-1, -1);
		switch (nInNegHalfspace)
		{
			case 0: // check for edge intersections
				polyVerts = checkForEdgeCylinderIntersections(polyVerts, Cxyz,
			...
			case 1: // compute and return a trimmed triangle
				...
				polyVerts = computeTrimmedPoly(polyVerts, Cxyz, uHat, vHat,
						uRadius, vRadius, nInNegHalfspace, be);
				break;
	...
	private Vec4[] checkForEdgeCylinderIntersections(Vec4[] polyVerts,
			Vec4 Cxyz, Vec4 uHat, Vec4 vHat, double uRadius, double vRadius)
	{
	...
	private Vec4[] computeTrimmedPoly(Vec4[] polyVerts, Vec4 Cxyz, Vec4 uHat,
			Vec4 vHat, double uRadius, double vRadius, int nInside,
			SectorGeometry.BoundaryEdge be)
	{
		...
		Vec4 p1 = intersectWithEllCyl(polyVerts[0], polyVerts[1], Cxyz, uHat,
				vHat, uRadius, vRadius);
		Vec4 p2 = intersectWithEllCyl(polyVerts[0], polyVerts[2], Cxyz, uHat,
				vHat, uRadius, vRadius);
	...
	private Vec4 intersectWithEllCyl(Vec4 v0, Vec4 v1, Vec4 Cxyz, Vec4 uHat,
			Vec4 vHat, double uRadius, double vRadius)
	{
		...
		double a = v1xMinusV0x * v1xMinusV0x / uRsquared + v1yMinusV0y
				* v1yMinusV0y / vRsquared;
		double b = 2.0 * (v0x * v1xMinusV0x / uRsquared + v0y * v1yMinusV0y
				/ vRsquared);
	...
	public static class RectGeometry
	{
		...
		{
			return (int) Math
					.floor((longitude - tile.sector.getMinLongitude().degrees)
							* colFactor);
		}
		...
		{
			return (int) Math
					.floor((latitude - tile.sector.getMinLatitude().degrees)
							* rowFactor);
		}
		...
		public double[] getPointAt(double lat, double lon)
		{
			...
			ret[0] = c0[0] * (1. - u) * (1 - v) + c1[0] * (u) * (1. - v)
					+ c2[0] * (1. - u) * (v) + c3[0] * u * v + refCenter[0];
			ret[1] = c0[1] * (1. - u) * (1 - v) + c1[1] * (u) * (1. - v)
					+ c2[1] * (1. - u) * (v) + c3[1] * u * v + refCenter[1];
			ret[2] = c0[2] * (1. - u) * (1 - v) + c1[2] * (u) * (1. - v)
					+ c2[2] * (1. - u) * (v) + c3[2] * u * v + refCenter[2];
			return ret;
		}
	...
	public static RectGeometry getTerrainGeometry(SectorGeometry tile)
	{
		if (tile == null || !(tile instanceof RectTile))
			...
			throw new IllegalArgumentException(
					"SectorGeometry instance not of type RectTile");
