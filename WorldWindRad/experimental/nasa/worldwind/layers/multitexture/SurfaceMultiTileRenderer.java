 ...
 */
public abstract class SurfaceMultiTileRenderer extends SurfaceTileRenderer
		implements Disposable
{
	...
	{
		renderTile(dc, tile, 1);
	}

	public void renderTile(DrawContext dc, SurfaceTile tile, int textureUnits)
	{
		if (tile == null)
		{
		...
		List<SurfaceTile> al = new ArrayList<SurfaceTile>(1);
		al.add(tile);
	...
	abstract protected void preComputeTransform(DrawContext dc,
			SectorGeometry sg);

	abstract protected void computeTransform(DrawContext dc, SurfaceTile tile,
			Transform t);

	abstract protected Iterable<SurfaceTile> getIntersectingTiles(
			DrawContext dc, SectorGeometry sg,
			...
			Iterable<? extends SurfaceTile> tiles)
	{
		...
		try
		{
			...
			boolean showOutlines = this.showImageTileOutlines
					&& dc.getNumTextureUnits() > 2;
			...
			int texUnitsEnabled = numTexUnitsUsed;
			...
			{
				Iterable<SurfaceTile> tilesToRender = this
						.getIntersectingTiles(dc, sg, tiles);
				...
				{
					boolean bound = false;
					int currentTexUnits = numTexUnitsUsed;
					if (tile instanceof MultiTextureTile)
					{
						MultiTextureTile mtt = (MultiTextureTile) tile;
						currentTexUnits += mtt.getTextureCount() - 1;
						bound = mtt.bind(dc, GL.GL_TEXTURE0 + numTexUnitsUsed);
					}
					else
					{
						gl.glActiveTexture(GL.GL_TEXTURE0);
					...
					if (bound)
					{
						...
						if (showOutlines)
						{
							...
							gl.glLoadIdentity();
							gl.glScaled(transform.HScale, transform.VScale, 1d);
							gl.glTranslated(transform.HShift, transform.VShift,
									0d);
						}
						...
						//enable the amount of texture units required
						while (texUnitsEnabled < currentTexUnits)
						{
							gl
						...
						sg.renderMultiTexture(dc, currentTexUnits);
					}
				}
			}

			for (int i = 0; i < texUnitsEnabled; i++)
			{
		...
		catch (Exception e)
		{
			Logging.logger().log(
					Level.SEVERE,
					Logging.getMessage("generic.ExceptionWhileRenderingLayer",
	...
	private void initAlphaTexture(int size)
	{
		...
		TextureData textureData = new TextureData(GL.GL_ALPHA, size, size, 0,
				GL.GL_ALPHA, GL.GL_UNSIGNED_BYTE, false, false, false,
				textureBytes.rewind(), null);
		this.alphaTexture = TextureIO.newTexture(textureData);

		this.alphaTexture.bind();
		this.alphaTexture.setTexParameteri(GL.GL_TEXTURE_MAG_FILTER,
				GL.GL_NEAREST);
		this.alphaTexture.setTexParameteri(GL.GL_TEXTURE_MIN_FILTER,
				GL.GL_NEAREST);
		this.alphaTexture.setTexParameteri(GL.GL_TEXTURE_WRAP_S,
				GL.GL_CLAMP_TO_BORDER);
		this.alphaTexture.setTexParameteri(GL.GL_TEXTURE_WRAP_T,
				GL.GL_CLAMP_TO_BORDER);
	...
	private void initOutlineTexture(int size)
	{
		...
		TextureData textureData = new TextureData(GL.GL_LUMINANCE, size, size,
				0, GL.GL_LUMINANCE, GL.GL_UNSIGNED_BYTE, false, false, false,
				textureBytes.rewind(), null);
		this.outlineTexture = TextureIO.newTexture(textureData);

		this.outlineTexture.bind();
		this.outlineTexture.setTexParameteri(GL.GL_TEXTURE_MAG_FILTER,
				GL.GL_NEAREST);
		this.outlineTexture.setTexParameteri(GL.GL_TEXTURE_MIN_FILTER,
				GL.GL_NEAREST);
		this.outlineTexture.setTexParameteri(GL.GL_TEXTURE_WRAP_S,
				GL.GL_CLAMP_TO_EDGE);
		this.outlineTexture.setTexParameteri(GL.GL_TEXTURE_WRAP_T,
				GL.GL_CLAMP_TO_EDGE);
	}
}
