...
public class NormalTessellator extends WWObjectImpl implements Tessellator
{
    protected static class RenderInfo
    {
        ...
        private RenderInfo(int density, DoubleBuffer vertices, DoubleBuffer normals, DoubleBuffer texCoords, Vec4 refCenter, int resolution)
        {
    ...
    public static class RectTile implements SectorGeometry
    {
        ...
        public RectTile(NormalTessellator tessellator, Extent extent, int level, int density, Sector sector,
                        double cellSize)
        {
    ...
    private static class CacheKey
    {
        ...
        public CacheKey(Globe globe, Sector sector, double verticalExaggeration, int resolution, int density)
        {
            this.sector = sector;
            this.resolution = resolution;
            this.density = density;
    ...
    private NormalTessellator.CacheKey createCacheKey(DrawContext dc, RectTile tile, int resolution)
    {
        return new CacheKey(dc.getGlobe(), tile.sector, dc.getVerticalExaggeration(), resolution, tile.density);
    }
    ...
    public RenderInfo buildVerts(DrawContext dc, RectTile tile, int resolution, boolean makeSkirts)
    {
        ...
        java.nio.DoubleBuffer norms = BufferUtil.newDoubleBuffer(numVertices * 3);
        ...
        for (int j = 0; j <= density + 2; j++)
        {
            ...
            for (int i = 0; i <= density + 2; i++)
            {
                ...
                Vec4 n = globe.computeSurfaceNormalAtPoint(p);
                norms.put(in++, n.x).put(in++, n.y).put(in++, n.z);
        ...
        return new RenderInfo(density, verts, norms, getTextureCoordinates(density), refCenter, elevations.getResolution());
    }
    ...
    private PickedObject[] pick(DrawContext dc, RectTile tile, List<Point> pickPoints)
    {
        if (dc == null)
        {
            String msg = Logging.getMessage("nullValue.DrawContextIsNull");
