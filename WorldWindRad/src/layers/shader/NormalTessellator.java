...
public class NormalTessellator extends WWObjectImpl implements Tessellator
{
	protected static class RenderInfo
	{
				...
				DoubleBuffer normals, Vec4 refCenter, int resolution)
		{
			...
			this.normals = normals == null ? getNormals(density, vertices,
					refCenter) : normals;
			...
			boolean makeSkirts)
	{
		...
		java.nio.DoubleBuffer norms = BufferUtil
				.newDoubleBuffer(numVertices * 3);
			...
			DoubleBuffer vertices, Vec4 referenceCenter)
	{
		...
		for (int j = 1; j < side - 1; j++)
		{
			for (int i = 1; i < side - 1; i++)
			{
				...
				p1 = (i == 1) ? null : getVec4(index1, vertices,
						referenceCenter);
				p2 = (i == density + 1) ? null : getVec4(index2, vertices,
						referenceCenter);
				p3 = (j == 1) ? null : getVec4(index3, vertices,
						referenceCenter);
				p4 = (j == density + 1) ? null : getVec4(index4, vertices,
						referenceCenter);

				if (p1 == null)
					p1 = p0.add3(p0.subtract3(p2));
				if (p2 == null)
					p2 = p0.add3(p0.subtract3(p1));
				if (p3 == null)
					p3 = p0.add3(p0.subtract3(p4));
				if (p4 == null)
					p4 = p0.add3(p0.subtract3(p3));
