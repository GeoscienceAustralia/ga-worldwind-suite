...
import au.gov.ga.worldwind.layers.mercator.BasicMercatorTiledImageLayer;
import au.gov.ga.worldwind.layers.mercator.MercatorTiledImageLayer;
		...
		RetrievalService, Thread.UncaughtExceptionHandler
{
	...
	private static final String RUNNING_THREAD_NAME_PREFIX = Logging
			.getMessage("BasicRetrievalService.RunningThreadNamePrefix");
	private static final String IDLE_THREAD_NAME_PREFIX = Logging
			...
			RetrievalFuture, Comparable<RetrievalTask>
	{
		...
		/**
		 * @param that
		 *            the task to compare with this one
	...
	public void uncaughtException(Thread thread, Throwable throwable)
	{
		Logging
				...
				.fine(
						Logging
								.getMessage(
										"BasicRetrievalService.UncaughtExceptionDuringRetrieval",
										thread.getName()));
	}
								...
								ThreadPoolExecutor threadPoolExecutor)
						{
							...
							Logging
									...
									.finer(
											Logging
													.getMessage(
															"BasicRetrievalService.ResourceRejected",
															((RetrievalTask) runnable)
																	.getRetriever()
																	.getName()));

							super.rejectedExecution(runnable,
									threadPoolExecutor);
						}
					});

			this.staleRequestLimit = Configuration.getLongValue(
			...
			if (task.retriever.getBeginTime() - task.retriever.getSubmitTime() > limit)
			{
				...
				Logging
						...
						.finer(
								Logging
										.getMessage(
												"BasicRetrievalService.CancellingTooOldRetrieval",
	...
	public ExtendedRetrievalService()
	{
		...
		this.queueSize = Configuration.getIntegerValue(
				AVKey.RETRIEVAL_QUEUE_SIZE, DEFAULT_QUEUE_SIZE);
			...
			double priority)
	{
		...
		if (this.activeTasks.contains(task)
				|| this.executor.getQueue().contains(task))
			return null;
	...
	public boolean contains(Retriever retriever)
	{
		...
		return (this.activeTasks.contains(task) || this.executor.getQueue()
				.contains(task));
	}
	...
	public double getProgress()
	{
						...
						.log(
								...
								Logging
										.getMessage(
												...
												retriever.getName() != null ? retriever
														.getName()
														: ""), e);
			}
		}
		...
		if (totalContentLength < 1)
			...
			progress = Math.min(100.0, 100.0 * (double) totalBytesRead
