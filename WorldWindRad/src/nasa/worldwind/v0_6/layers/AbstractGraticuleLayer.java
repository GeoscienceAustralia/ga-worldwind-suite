...
All Rights Reserved.
*/
package nasa.worldwind.v0_6.layers;
...
public class AbstractGraticuleLayer extends AbstractLayer
{
    ...
    protected boolean needsToUpdate(DrawContext dc)
    {
        ...
        double heading = 0d, pitch = 0d;
        if(view instanceof OrbitView)
        {
        	heading = ((OrbitView)view).getHeading().degrees;
        	pitch = ((OrbitView)view).getPitch().degrees;
        }

        if (Math.abs(this.lastViewHeading - heading) > 1)
            return true;
        if (Math.abs(this.lastViewPitch - pitch) > 1)
            return true;
    ...
    protected void clear(DrawContext dc)
    {
        ...
        {
        	heading = ((OrbitView)dc.getView()).getHeading().degrees;
        	pitch = ((OrbitView)dc.getView()).getPitch().degrees;
        }
        
        this.lastViewHeading = heading;
