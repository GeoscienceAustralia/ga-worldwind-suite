...
/**
 * Adapter used for displaying layer nodes in a ListView.
 * 
 * @author Michael de Hoog (michael.dehoog@ga.gov.au)
 */
public class LayerListAdapter extends AbstractItemListAdapter
{
	public LayerListAdapter(ItemModel model, Item item, LayoutInflater inflater)
	{
	...
	protected String getText(Item item)
	{
		...
		if (item.isLeaf() && item.getOpacity() != 1)
		{
			s += " (" + (int) (item.getOpacity() * 100) + "%)";
		}
		return s;
	}
}
