 ...
 * @author head
 */
public class EnvelopeShapeFileReader extends ShapeFileStream {
     ...
     */
    public EnvelopeShapeFileReader(String inFilename) throws FileNotFoundException, IOException {
        super(inFilename);
...
//            java.nio.channels.FileChannel channel = new RandomAccessFile(myFilename+myShpExt, "r").getChannel();
            
            // Create an inputstream on the channel
//            InputStream is = java.nio.channels.Channels.newInputStream(channel);
//            myShpStream = new LEDataInputStream(is);
     ...
     */
    public ShapeFileRecord read(Envelope inEnvelope) throws IOException {
        ShapeFileRecord tempRecord = null;
        Shape tempShape = null;
        while((tempShape == null) && (myReadPosition < myHeader.getFileLength())){
            // Bytes 0 to 4 represent the record number in the file, these may
            // be out of order.
            myShpStream.setLittleEndianMode(false);
            int tempShapeIndex = myShpStream.readInt();
            ...
            // retrieve that shape.
            tempShape = readShape(tempShapeType, tempContentLength, myShpStream, inEnvelope);
            ...
            if (tempShape != null){
                ...
                dbRecord.setShape(tempShape);
                tempRecord = new ShapeFileRecord(tempShapeIndex, dbRecord);
            }
            else{
                ((DbaseFileReader) myDbaseStream).skip();
            }
        }
        return tempRecord;
    }
     ...
     */
    private final Shape readShape(int inShapeType, int inContentLength, LEDataInputStream in, Envelope inEnvelope) throws IOException {
        Shape rShape = null;
        switch (inShapeType) {
            ...
            case ShapeFile.SHAPE_POINT:
                rShape = readPoint(in, inEnvelope);
                break;
                
            case ShapeFile.SHAPE_POLYLINE:
                rShape = readPolyLine(in, inContentLength, inEnvelope);
                break;
                
            case ShapeFile.SHAPE_POLYGON:
                // Polygon -> converts to MultiPolygon
                rShape = readPolygon(in, inContentLength, inEnvelope);
                break;
                
            case ShapeFile.SHAPE_MULTIPOINT:
                rShape = readMultiPoint(in, inContentLength, inEnvelope);
                break;
            ...
            case ShapeFile.SHAPE_POINTM:
                rShape = readPointM(in, inEnvelope);
                break;
                
            case ShapeFile.SHAPE_POLYLINEM:
                rShape = readPolyLineM(in, inContentLength, inEnvelope);
                break;
                
                //25 PolygonM
                
            case ShapeFile.SHAPE_MULTIPOINTM:
                rShape = readMultiPointM(in, inContentLength, inEnvelope);
                break;
     ...
     */
    private final Point readPoint(LEDataInputStream in, Envelope inEnvelope) throws IOException {
        // create a new point
        Point tempPoint = new Point(0,0);
        
        readPoint(tempPoint, in);
        
        // ensure this point intersects these exetnts.
        if (!inEnvelope.overlaps(tempPoint.getEnvelope())) return null;
     ...
     * Reads the Point from the shape file.
     */
    private final Point readPoint(LEDataInputStream in) throws IOException {
        // create a new point
        Point tempPoint = new Point(0,0);
     ...
     * Reads the Point from the shape file.
     */
    private final Point readPoint(Point inPoint, LEDataInputStream in) throws IOException {
    ...
    /** Reads the envelope from the shape file, returns true if they overlap.*/
    private final boolean readEnvelope(LEDataInputStream in, Envelope inEnvelope) throws IOException{
        ...
        if (inEnvelope.getMinX() > tempXmax) return false;
        if (inEnvelope.getMaxY() < tempYmin) return false;
        
        // eliminate rectangles which do not overlap.
        if (inEnvelope.getMaxX() < tempXmin) return false;
        if (inEnvelope.getMinY() > tempYmax) return false;
        return true;
    }
     ...
     */
    private final MultiPoint readMultiPoint(LEDataInputStream in, int inContentLength, Envelope inEnvelope) throws IOException{
        
        // bytes 1 to 4 are the type and have already been read.
        
        // bytes 4 to 36 are the bounding box of the multi points
        // the MultiPoint will calculate the envelope on creation.
        if (!readEnvelope(in, inEnvelope)){
            in.skipBytes(inContentLength*2-36);
            return null;
        }
     ...
     * Reads the MultiPoint from the shape file.
     */
    private final MultiPointM readMultiPointM(LEDataInputStream in, int inContentLength, Envelope inEnvelope) throws IOException{
     ...
     * Reads the MultiPoint from the shape file.
     */
    private final MultiPointZ readMultiPointZ(LEDataInputStream in, int inContentLength, Envelope inEnvelope) throws IOException{
     ...
     * Reads the Point from the shape file.
     */
    private final PointM readPointM(LEDataInputStream in, Envelope inEnvelope) throws IOException{
        ...
        // return the new Point
        Envelope tempEnvelope = readEnvelope(in);
        
        // ensure this Envelope intersect.
        if (!inEnvelope.overlaps(tempPoint.getEnvelope())){
            return null;
        }
        return tempPoint;
     ...
     * Reads the Point from the shape file.
     */
    private final PointZ readPointZ(LEDataInputStream in, Envelope inEnvelope) throws IOException{
     ...
     */
    private final MultiPolygon readPolygon(LEDataInputStream in, int inContentLength, Envelope inEnvelope) throws IOException{
     ...
     * Reads the Polygon from the shape file.
     */
    private final MultiPolygonM readPolygonM(LEDataInputStream in, int inContentLength, Envelope inEnvelope) throws IOException{
     ...
     * Reads the Polygon from the shape file.
     */
    private final MultiPolygonZ readPolygonZ(LEDataInputStream in, int inContentLength, Envelope inEnvelope) throws IOException{
     ...
     */
    private final MultiLineString readPolyLine(LEDataInputStream in, int inContentLength, Envelope inEnvelope) throws IOException{
     ...
     */
    private final MultiLineStringM readPolyLineM(LEDataInputStream in, int inContentLength, Envelope inEnvelope) throws IOException{
     ...
     */
    private final MultiLineStringZ readPolyLineZ(LEDataInputStream in, int inContentLength, Envelope inEnvelope) throws IOException{
